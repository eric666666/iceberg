...
abstract class MergingSnapshotProducer<ThisT> extends SnapshotProducer<ThisT> {
  ...
  private void validateNoNewDeletesForDataFiles(
    ...
    for (DataFile dataFile : dataFiles) {
      ...
      LOG.warn(
          "Check delete files for commit snapshot.parent[{}], parent.sequenceNumber()[{}] ,base.snapshot(startingSnapshotId).sequenceNumber()[{}] ,startingSequenceNumber[{}]",
          parent,
      ...
      if (base.snapshot(startingSnapshotId).sequenceNumber() == startingSequenceNumber) {
        ...
        LOG.warn(
            "Skip delete files check on commit rewrite files. parent.sequenceNumber()[{}] == startingSequenceNumber[{}]",
            parent.sequenceNumber(),
            startingSequenceNumber);
        return;
      }
